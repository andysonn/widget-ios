...
const apiKey = "784b836db08124e0c023dd686ac02615"
...
const widgetPreview = ["small", "medium", "large"][0]
...
const textFormat = {
  ...
  largeDate1:  { size: 26, color: "", font: "light" },
  largeDate2:  { size: 20, color: "", font: "light" },
  ...
  sunrise:     { size: 16, color: "", font: "medium" },
}
...
async function lunar(column) {
  const url = 'https://api.xlongwei.com/service/datetime/convert.json'
  const req = new Request(url)
  const res = await req.loadJSON()
  const lunarText = `${res.chinese.replace(/.*年/, '')}`

  let lunarStack = align(column)
  lunarStack.setPadding(padding, padding, padding, padding)
  provideText(lunarText, lunarStack, textFormat.smallDate)
}
